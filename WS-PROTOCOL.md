...
**Commands:** `file.create` (introduce this path into the tree as its own tick, empty — fails if the path already has ticks), `chat.append`, `edit.atom`, `delete.ticks` (batch, `targets: string[]` — generic over any tick kind on this path's chain: an atom, or an annotation — note, prompt, summary occurrence, ask, image), `move.atom` (generic over any tick kind too, despite the name — kept distinct from the branch connection's own `move.tick` to avoid two different commands sharing one name), `delete`, `chat.writer`, `chat.fixer`, `chat.note`, `enter.scene`, `leave.scene`, `at` (rebase wrapper — any command above, including `enter.scene`/`leave.scene`, can be sent wrapped in `at` to run as of a historical tick).  
