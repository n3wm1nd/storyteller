...
--
-- The tool the model is given is deliberately narrow: it takes exactly one
-- parameter, the replacement text, and the target atom (its tick id and
-- file path) is bound into the tool by the caller rather than supplied by
-- the model — so the model has no way to ever target the wrong atom. This
-- is what @Storyteller.Agent.Fix@ and @Storyteller.Agent.FlowWrite@ both
-- reduce to: "here is one atom, here is an instruction, does it need to
...
--   so the only thing the model supplies is the replacement text.
replaceAtomTool
  ...
  => TickId -> FilePath -> T.Text -> Sem r ReplaceResult
replaceAtomTool tid path newText = do
  ...
  let tool = mkToolWithMeta
      ...
      prompt = "Atom under review:\n\n" <> content
             ...
             <> "\n\nIf this atom needs to change because of the instruction, call replace_atom \
                \with the corrected text. If it is already fine as-is, just reply briefly and do \
                \not call the tool."
