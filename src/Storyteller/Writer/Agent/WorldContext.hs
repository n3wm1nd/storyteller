...
-- hand-authors (notes, places, events, world-building) versus the one
-- reserved file meant as standing instruction to the model (a style
-- guide). Both are drawn from the same lore-eligible file set
-- ('Storyteller.Writer.Lore.isLoreEligible') by a single path-convention
-- split, not two independent classification schemes -- 'style.md' at the
-- branch root is 'SystemContext', everything else 'isLoreEligible' accepts
-- is 'WorldLore'.
--
...
--   mirroring the @sheet.md@\/@journal.md@ convention 'isLoreEligible'
--   already special-cases for a character branch. Everything else
--   'isLoreEligible' accepts is 'WorldLore' instead.
isSystemContextPath :: FilePath -> Bool
isSystemContextPath path = takeFileName path == "style.md" && takeDirectory path == "."

-- | The story's freeform reference material -- notes, places, events,
--   anything a user hand-authored that isn't a chapter, isn't chat
--   scratch, and isn't the reserved 'isSystemContextPath' style guide.
--   Meant to be read as reference material the model reasons over, not
--   instruction -- belongs early in a chapter's history, ahead of the
--   chapter prose itself, distinct from 'SystemContext' which belongs in
...
-- | Both halves of the story-wide split, one dispatch. Oldest-alphabetical
--   order within each half (there's no chapter-number ordering to apply
--   here, same reasoning as 'Storyteller.Writer.Lore.buildLoreTree').
worldContextOf :: forall m. Core.StoreM m => Core.StoreT m (WorldLore, SystemContext)
worldContextOf = do
  files <- List.sort . filter isLoreEligible <$> FS.list
