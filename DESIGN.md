...
**Roleplay** (`Roleplay.hs`) — the interrogate-then-write alternative to Write/FlowWrite, reached as its own mode in the same per-message agent selector (see Interaction Modes below), not a separate surface. `roleplayAgent` iterates every character present in the scene (presence ticks decide who, same as ordinary Write's identity block), asks each one a targeted question via `characterIntentAgent` — a real tool-calling subagent scoped to that character's own branch, primed with their full uncurated context (sheet, whole journal, everything else they've tracked), with broad glob/read/write/edit access to their own branch and read-only access to shared lore — then composes one coherent scene from all their answers in a final plain call. Once the scene lands, every present character (asked or not) gets a post-scene `characterReflectAgent` pass: their own account of what just happened, filtered to what they could plausibly perceive, appended to their own `journal.md` with a cross-branch ref back to the scene atom. This is the real, working shape of "NPCs only know what their branches contain" for actual back-and-forth scenes, not just narration *about* a character — closer to the standalone RP surface's premise (see README.md) than anything else built so far, though it's still one mode inside Writer's own agent selector, not a separate surface.
...
Selection *within* this — which of a branch's other files count as lore, which count as a chapter, what a given character's context looks like — is defined by a small, purpose-built **Context DSL** (`src/Storyteller/Context/DSL/`, fully specified in **[CONTEXT-DSL.md](CONTEXT-DSL.md)**), not a heuristic the backend guesses at and not a fixed bucket/glob picker. `context.writer` (the definition backing the list above), `context.lore`, `context.chapters`, `context.other`, and `context.character` are ordinary named DSL definitions, each composing the others by name rather than by Haskell parameter-passing — a project can override any one of them independently just by committing replacement DSL text to a dedicated `contexts` branch, and `writeAgent`'s call site never has to know or care whether it got the compiled-in default or a project's own override. The same override mechanism is exposed per-request too: a client can submit its own Context DSL program alongside a single generation call, which replaces `context.writer`'s result for that call only, nothing written anywhere. The frontend's casual context panel (toggle lore/chapters/style on or off, add a character or an extra file) is a client-side synthesizer that compiles those choices into exactly this kind of program; a power-user DSL editor sits next to it for anyone who wants to write or save the program directly. There is no relevance-ranking routing agent and no semantic/RAG selection: everything a definition's own body reaches for is read in full, every call. That's the real, current form of what this section used to describe as a routing/filter agent split — deliberately simpler, and the one place this architecture doesn't yet have an answer for what happens once a story's total context stops fitting in the window at all (see Open Questions).
