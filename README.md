...
What counts as "lore," a "chapter," or a given character's context isn't hardcoded — it's defined by a small DSL (see `CONTEXT-DSL.md`) that a project can override on its own branch, or a single request can override just for that call. The assembly logic is data, not a fixed pipeline.
...
Not built yet, despite being described in `DESIGN.md`: semantic/RAG search, the consistency-checker/merge/synthesis/task-tracking agents, manuscript import and EPUB/PDF export, and the RP/Wiki surfaces mentioned above. Treat anything in `DESIGN.md` not corroborated by this section as a direction, not a status.
