    ...
    it "keeps the journal Summary tick out of journal.md's own real chain, relinking around it" $ do
      ...
      case result of
        ...
        Right (upd, _sig) -> do
          ...
          -- (the last real atom it covers -- see summaryTicksFor's own
          ...
          all ((== 1) . length . wtRefs) summaryTicks `shouldBe` True
    ...
    it "two sequential chapter-summary passes produce two summary WireTicks with distinct, real anchor ids" $ do
      ...
      case result of
        ...
        Right (upd, _sig) -> do
          ...
          -- Each occurrence's own anchor is its single ref; its two
          -- boundaries (the previous occurrence's own anchor and alt-chain
          -- tip) are plain fields, present only once an earlier occurrence
          -- exists -- named directly, never re-derived by the client.
          let anchors = map (\wt -> case wtRefs wt of [a] -> Just a; _ -> Nothing) summaryTicks
          ...
          (case (summaryTicks, anchors) of
             ([first, second], [firstAnchor, _]) -> do
               lookup "lowerBound"  (wtFields first)  `shouldBe` Nothing
               lookup "prevAltHead" (wtFields first)  `shouldBe` Nothing
               lookup "lowerBound"  (wtFields second) `shouldBe` firstAnchor
               lookup "prevAltHead" (wtFields second) `shouldSatisfy` (/= Nothing)
             _ -> expectationFailure "expected exactly two summary ticks")
