...
-- | Does 'writeAgent''s own internal tick-history gathering -- this
--   chapter's own conversation so far, reconstructed via
...
--   'Server.Writer.File.chatWriter''s own store-prompt-then-generate
...
spec runner = describe "same-file conversation history reaching the writer (real LLM, cached)" $
  it "keeps a second turn consistent with a detail only the first turn's own reply established" $
    runExpect @judgeModel runner $ do
      -- The file has to exist before writeAgent's own internal
      -- fileTicksOf read finds anything to attach ticks to -- same
      -- requirement 'Agent.Integration.CharacterPresenceSpec' documents.
      _ <- runStorage @Main (Ops.addAtom scenePath "")
      historySoFar1 <- runStorage @Main (Tick.fileTicksOf scenePath)

      -- Turn one: generate, store the prompt, then persist the result as
      -- an atom -- exactly what turns this into a real prior turn for
      -- turn two to see. This turn's own prompt must be stored *after*
      -- writeAgent's own call, not before -- see 'Server.Writer.File.
      ...
      -- own prompt/reply pair, read internally by writeAgent's own call
      -- below -- confirmed non-empty here first so the test still asserts
      -- turn one's own history actually landed.
