...
-- | Every editing operation is now a plain 'SM.StorageT' computation, so a
--   whole test body is just one such computation, dispatched once against
--   the branch — no Polysemy 'At'/nesting needed anywhere.
runEdit
...
-- | For the handful of tests that specifically need the cross-branch
...
storeN :: SM.StorageM m => Int -> SM.StorageT m [TickId]
storeN n = mapM (\i -> SM.store (draft (T.pack ("t" <> show i)))) [1..n]

-- | Collect all tick messages oldest-first (skipping the root tick).
chainMessages :: SM.StorageM m => SM.StorageT m [T.Text]
chainMessages = do
...
chainPairs :: SM.StorageM m => SM.StorageT m [(TickId, T.Text)]
chainPairs = do
  ticks <- SM.followChain [] (\acc t -> (t : acc, tickParent t))
...
atomMessages :: SM.StorageM m => SM.StorageT m [T.Text]
atomMessages = do
  ticks <- SM.followChain [] (\acc t -> (t : acc, tickParent t))
...
spec = do
  ...
  describe "popTick" $ do
    it "captures head's message and refs" $ do
      ...
      let result = runEdit $ do
            ...
            t3 <- SM.store (TickData { tickRefs = refs, tickFields = [], tickMessage = "annotated" })
            -- at t3: t3 is at head, pop it
            (d, _) <- SM.atChecked True t3 SM.popTick
            return d
      case result of
        Left err -> fail err
        Right d  -> do
          SM.tdMessage d `shouldBe` "annotated"
          length (SM.tdRefs d) `shouldBe` 2

    it "pop then push at same position is identity on messages" $ do
      -- at t2: pop t2, push it back. at replays t3 on top → same chain.
      let result = runEdit $ do
            [_t1, t2, _t3] <- storeN 3
            _ <- SM.atChecked True t2 $ do
              d <- SM.popTick
              _ <- SM.pushTick d
              return ()
            chainMessages
  ...
  describe "moveTick backward" $ do
    it "moves last tick to front" $ do
      let result = runEdit $ do
            [_t1, _t2, t3] <- storeN 3
            mapping <- SM.moveTick t3 Nothing
            msgs <- chainMessages
            return (msgs, mapping)
      case result of
        Left err -> fail err
    ...
    it "moves last tick to middle" $ do
      let result = runEdit $ do
            [t1, _t2, t3] <- storeN 3
            _mapping <- SM.moveTick t3 (Just t1)
            chainMessages
  ...
  describe "moveTick identity" $ do
    it "moving tick to its own position is a no-op on messages" $ do
      let result = runEdit $ do
            [t1, t2, _t3] <- storeN 3
            _ <- SM.moveTick t2 (Just t1)  -- t2 is already after t1
            chainMessages
  ...
  describe "moveTick id mapping" $ do
    ...
    it "mapping has no duplicate old ids" $ do
      let result = runEdit $ do
            [t1, _t2, t3] <- storeN 3
            mapping <- SM.moveTick t3 (Just t1)
  ...
  describe "moveTick errors" $ do
    it "fails if tid not in chain" $ do
      let result = runEdit $ do
            _ <- storeN 3
            SM.moveTick (TickId "nonexistent") Nothing
      case result of
    ...
    it "fails if afterTickId not in chain" $ do
      let result = runEdit $ do
            [t1, _t2, _t3] <- storeN 3
            SM.moveTick t1 (Just (TickId "nonexistent"))
      case result of
        Left err -> err `shouldContain` "not found"
        Right _  -> fail "expected failure"
    ...
    it "fails if move would place tick before its ref" $ do
      ...
      let result = runEdit $ do
            [_t1, _t2, t3] <- storeN 3
            t2ref <- SM.store (TickData { tickRefs = [t3], tickFields = [], tickMessage = "note" })
            SM.moveTick t2ref Nothing  -- would place note before t3
      case result of
    ...
    it "fails if move would place tick after its referencing tick" $ do
      ...
      let result = runEdit $ do
            [t1, _t2] <- storeN 2
            t2note <- SM.store (TickData { tickRefs = [t1], tickFields = [], tickMessage = "note" })
            SM.moveTick t1 (Just t2note)  -- t1 after t2note which refs t1
      case result of
  ...
  describe "moveTick note ref" $ do
    it "note retains its ref when moved down one place" $ do
      ...
      let result = runEdit $ do
            [_a1, a2, _a3] <- storeN 3
            n1 <- SM.store (TickData { tickRefs = [a2], tickFields = [("type","note")], tickMessage = "a comment" })
            mapping <- SM.moveTick n1 (Just a2)
            let newN1 = maybe n1 id (lookup n1 mapping)
                newA2 = maybe a2 id (lookup a2 mapping)
            (tick, _) <- SM.atChecked False newN1 SM.getTick
  ...
  describe "moveTick file content" $ do
    it "backward move preserves correct file content order" $ do
      ...
      let result = runEdit $ do
            _t1 <- SM.appendAtom "scene.md" "para1\n"
            _t2 <- SM.appendAtom "scene.md" "para2\n"
            t3  <- SM.appendAtom "scene.md" "para3\n"
            _ <- SM.moveTick t3 Nothing
    ...
    it "forward move preserves correct file content order" $ do
      ...
      let result = runEdit $ do
            t1 <- SM.appendAtom "scene.md" "para1\n"
            _t2 <- SM.appendAtom "scene.md" "para2\n"
            t3  <- SM.appendAtom "scene.md" "para3\n"
            _ <- SM.moveTick t1 (Just t3)
            SM.readFileS "scene.md"
    ...
    it "move to middle preserves correct file content order" $ do
      ...
      let result = runEdit $ do
            _ta <- SM.appendAtom "scene.md" "A\n"
            tb  <- SM.appendAtom "scene.md" "B\n"
            _tc <- SM.appendAtom "scene.md" "C\n"
            td  <- SM.appendAtom "scene.md" "D\n"
            _ <- SM.moveTick td (Just tb)
            SM.readFileS "scene.md"
      case result of
        Left err -> fail err
    ...
    it "QuickCheck: file content is a permutation of original paragraphs" $
      property $ \(Positive n) (NonNegative from) (NonNegative to) ->
        ...
        in
        let result = runEdit $ do
              ids <- mapM (\i -> SM.appendAtom "f.md" (T.pack ("p" <> show i <> "\n"))) [1..n']
              let tid    = ids !! from'
                  mAfter = if to' == 0 then Nothing else Just (ids !! (to' - 1))
              _ <- SM.moveTick tid mAfter
              SM.readFileS "f.md"
  ...
  describe "mergeAtoms" $ do
    it "merges a contiguous run into one atom with concatenated content" $ do
      let result = runEdit $ do
            t1 <- SM.append "scene.md" "para1\n"
            t2 <- SM.append "scene.md" "para2\n"
            t3 <- SM.append "scene.md" "para3\n"
            (_, mapping) <- SM.mergeAtoms [t1, t2, t3]
            bs   <- SM.readFileS "scene.md"
            msgs <- atomMessages
    ...
    it "merges regardless of input order" $ do
      let result = runEdit $ do
            t1 <- SM.append "scene.md" "a\n"
            t2 <- SM.append "scene.md" "b\n"
            _ <- SM.mergeAtoms [t2, t1]
            SM.readFileS "scene.md"
    ...
    it "merges a run that isn't the whole chain, leaving the tail intact" $ do
      let result = runEdit $ do
            t1 <- SM.append "scene.md" "a\n"
            t2 <- SM.append "scene.md" "b\n"
            _t3 <- SM.append "scene.md" "c\n"
            _ <- SM.mergeAtoms [t1, t2]
            atomMessages
      result `shouldBe` Right ["a\nb\n", "c\n"]
    ...
    it "fails on a non-contiguous selection" $ do
      let result = runEdit $ do
            t1  <- SM.append "scene.md" "a\n"
            _t2 <- SM.append "scene.md" "b\n"
            t3  <- SM.append "scene.md" "c\n"
            SM.mergeAtoms [t1, t3]
      case result of
    ...
    it "a ref pointing at any merged atom remaps to the merged id" $ do
      ...
      -- *target* (t2) is itself being rewritten here, which triggers a
      -- second-generation cascade on n1 beyond the id 'mergeAtoms' itself
      -- reports: 'mergeAtoms' own tail-replay only ever rewrites a tick's
      -- *chain* parent, never a ref sitting further down its parent list —
      -- fixing those is 'updateReferences' cascade's job, a
      -- 'StoryStorage'-level concern layered on by
      -- 'Storyteller.Core.Git.runStorageEdit' (which every real caller
      -- goes through — see 'Server.Core.File.editFileAtom' et al.), so
      -- this needs the full stack, not a bare 'SM.mergeAtoms' call, to
      -- reproduce.
      let result = runEditBroadcast $ do
            (t1, t2) <- runStorage @Main $ do
              t1 <- SM.append "scene.md" "a\n"
              t2 <- SM.append "scene.md" "b\n"
              _t3 <- SM.append "scene.md" "c\n"
              _n1 <- SM.store (TickData { tickRefs = [t2], tickFields = [], tickMessage = "note" })
              return (t1, t2)
            (newTid, _) <- runStorageEdit @Main (SM.mergeAtoms [t1, t2])
            ticks <- runStorage @Main (SM.followChain [] (\acc t -> (t : acc, tickParent t)))
  ...
  describe "splitTick" $ do
    it "splits one atom into two in place, preserving file content" $ do
      let result = runEdit $ do
            _t1 <- SM.append "scene.md" "a\n"
            t2  <- SM.append "scene.md" "b\n\nc\n"
            (newIds, _) <- SM.splitTick t2 ["b\n\n", "c\n"]
            bs   <- SM.readFileS "scene.md"
            msgs <- atomMessages
    ...
    it "each resulting piece's own file-tree contribution is just that piece, not the whole atom or nothing" $ do
      ...
      -- accidentally cancel out across several commits. 'fileTicksOf' is
      -- what the frontend actually renders per atom (its 'ftContent' is a
      -- real tree-to-tree diff, not the stored message) — this is the one
      -- check that would have caught a splitTick that never wrote each
      -- piece's bytes into the working tree before committing it.
      let result = runEdit $ do
            _t1 <- SM.append "scene.md" "a\n"
            t2  <- SM.append "scene.md" "b\n\nc\n"
            (newIds, _) <- SM.splitTick t2 ["b\n\n", "c\n"]
            ticks <- SM.fileTicksOf "scene.md"
            return (mapMaybe (\tid -> SM.ftContent =<< find ((== tid) . SM.ftTickId) ticks)
                              (map unTickId newIds))
    ...
    it "a ref pointing at the original tick remaps to the first piece" $ do
      ...
      let result = runEditBroadcast $ do
            t1 <- runStorage @Main $ do
              t1 <- SM.append "scene.md" "a\n\nb\n"
              _t2 <- SM.append "scene.md" "c\n"
              _n1 <- SM.store (TickData { tickRefs = [t1], tickFields = [], tickMessage = "note" })
              return t1
            (newIds, _) <- runStorageEdit @Main (SM.splitTick t1 ["a\n\n", "b\n"])
            ticks <- runStorage @Main (SM.followChain [] (\acc t -> (t : acc, tickParent t)))
            let noteRefs = fmap (tickRefs . tickData) (find (\t -> tickMessage (tickData t) == "note") ticks)
    ...
    it "fails on a non-atom tick" $ do
      let result = runEdit $ do
            n1 <- SM.store (TickData { tickRefs = [], tickFields = [], tickMessage = "note" })
            SM.splitTick n1 ["x", "y"]
      case result of
